...
class PIIDataset(Dataset):
    def __init__(self, path: str, tokenizer, label_list: List[str], max_length: int = 256, is_train: bool = True, use_normalization: bool = False):
        self.items = []
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if self.use_normalization:
                    text, char_mapping = normalize_spoken_text(original_text)
                else:
                    text = original_text
...
def collate_batch(batch, pad_token_id: int, label_pad_id: int = -100):
    ...
    out = {
        ...
        "original_texts": [x.get("original_text", x["text"]) for x in batch],
        "char_mappings": [x.get("char_mapping", list(range(len(x["text"])))) for x in batch],
